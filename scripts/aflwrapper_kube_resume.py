...
if __name__ == "__main__":
    ...
    targz_path = Path(
        f"{SHARED_MOUNT_POINT}/tars_24hour_cgc_resume/{parsed_args.index}_{target_binary}.tar.gz"
    )
    ...
    if not Path("/results/").exists():
        l.info("Making /results/ folder..")
        makedirs("/results/")

    l.info(f"Copying '{targz_path}' to root..")
    copyfile(targz_path, f"/{target_binary}.tar.gz")
    l.info("Unzipping..")
    call(
        [
            "tar",
            "xf",
            f"/{target_binary}.tar.gz",
            "-C",
            "/",
        ]
    )
